...
List<String> values = [
  ...
  '8',
  '9'
]; //to define the values of gameboard
...
void game() {
  ...
  for (i = 0; i < 9; i++) {
    ...
    if (i.isEven) {
      ...
      int choise = int.parse(stdin.readLineSync()!); // to read the input
      if ((choise > 9) ||
          (values[choise - 1] == 'O') ||
      ...
      if (isWinner(values)) {
        ...
        print("winner winner chicken dinner the winner is: X");
        break;
      }
    } else {
      ...
      if (isWinner(values)) {
        ...
        print("winner winner chicken dinner the winner is: O");
        break;
      }
    }
